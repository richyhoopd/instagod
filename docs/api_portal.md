...
- `POST /brands/{slug}/plans/{pid}/generar` (202) — encola `plan.generar`. Acepta el plan en
  `temas`, en `error` (lote caído entero) y en `curacion` (un tema suelto que falló y se
  reaprobó); 422 en cualquier otro estado o sin temas pendientes, 409 si ya hay un job del plan
  vivo. Nunca repite lo hecho: solo toma los topics en `aprobado`, y los `generado` se quedan
  como están.
