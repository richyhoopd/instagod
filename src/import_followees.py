...
def importar(cuenta: str = "gdlscene", limite: int | None = None) -> None:
    cx = db.connect()
    try:
        db.init_db(cx)
        session = get_session()
        perfil = fetch_profile(session, cuenta.lstrip("@"))
        print(f"@{cuenta} sigue a {perfil['edge_follow']['count']} cuentas; listando…")
        cuentas = listar_following(session, perfil["id"], limite)
        ...
        print(f"❌ {exc} — IG limitó la sesión; reintenta en unas horas.")
    finally:
        cx.close()
